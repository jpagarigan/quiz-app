...
def get_question():
    dataset_choice = request.args.get('dataset', 'constants')
    ordered = request.args.get('ordered', 'false') == 'true'

    dataset = scientific_constants if dataset_choice == 'constants' else metric_conversions
    question_numbers = list(dataset.keys())

    if not ordered:
        random.shuffle(question_numbers)

    question_num = random.choice(question_numbers)
    correct_answer = dataset[question_num]

    # Generate 3 wrong choices
    all_choices = list(dataset.values())
    random.shuffle(all_choices)
    all_choices = [item for item in all_choices if item != correct_answer][:3]
    all_choices.append(correct_answer)
    random.shuffle(all_choices)

    return jsonify({
        'question_num': question_num,
        'question': f"What is the Constant or Conversion for {question_num}?",
        'choices': all_choices,
